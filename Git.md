...
* git rebase는 Git의 강력한 도구로, 브랜치 기반을 다른 브랜치 위로 옮겨서 커밋 히스토리를 보다 깔끔하게 정리하는 데 사용됩니다. merge와 같은 결과를 만들 수 있지만, 히스토리 구조가 달라집니다.
* git rebase는 **한 브랜치의 커밋들을 다른 브랜치의 마지막 커밋 뒤로 "재적용"**합니다.
* 결과적으로 커밋 그래프가 직선형(linear) 으로 정리됩니다.
* 협업 중인 공유 브랜치에서는 주의 필요 (히스토리 변경 발생)

### Rebaase vs Merge

| 항목      | `git merge`            | `git rebase`       |
| ------- | ---------------------- | ------------------ |
| 커밋 구조   | 분기 후 병합 (Fork + Merge) | 직선형으로 재정렬          |
| 커밋 수    | 보통 병합 커밋 1개 생성         | 커밋 수 유지 or 증가      |
| 히스토리    | 브랜치 흐름 보존              | 깔끔하고 단순함           |
| 충돌 발생 시 | 병합 시점에 한번 발생           | 커밋마다 개별 충돌 가능      |
| 사용 시기   | 협업 중 병합, 브랜치 히스토리 보존   | 개인 작업 정리, 깔끔한 히스토리 |

### 주요 명령어

| 명령어                     | 설명                                 |
| ----------------------- | ---------------------------------- |
| `git rebase 브랜치명`       | 현재 브랜치를 지정된 브랜치 위로 재배치             |
| `git rebase -i 브랜치명`    | 인터랙티브 모드로 커밋 정리 (squash, reword 등) |
| `git rebase --continue` | 충돌 해결 후 계속 진행                      |
| `git rebase --skip`     | 충돌 커밋 건너뜀                          |
| `git rebase --abort`    | 리베이스 중단 및 원래 상태로 복원                |
